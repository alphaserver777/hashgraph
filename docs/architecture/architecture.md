...
- Peer-registry теперь тоже имеет локальное SQLite-состояние узла: оператор может видеть, добавлять, исключать и удалять участников сети из веб-панели. При этом такое решение остаётся локальной operator-моделью конкретного узла, а не кластерным консенсусом по составу сети.
