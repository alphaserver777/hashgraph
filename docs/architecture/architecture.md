...
- Дополнительно есть отдельный Docker Compose профиль `linux-node` с сервисом `linux-node1` для первого вертикального среза Linux ingestion. Это технический bootstrap-контур, а не подтверждённый production deploy.
- В Compose:
