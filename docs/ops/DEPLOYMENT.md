...
- Следующий проектный production-контур для реальной среды предполагает container sidecar-агент на каждом Linux-хосте.
- Агент должен жить рядом с локальным узлом MDRJ-DAG, а не вместо него.
- Агент будет читать системные источники хоста (`journald`, `auth.log`, `sudo`) и отправлять нормализованные события в локальный HTTP API узла.
- На текущий момент это только проектное решение, а не уже реализованный deploy-контур.
- До реализации такого агента production deployment всё ещё считается неописанным.
