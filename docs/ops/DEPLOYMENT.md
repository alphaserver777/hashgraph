...
- Важный принцип:
  ...
  - compose-сервис `linux-node` передаёт эти env-переменные внутрь контейнера, чтобы тот же набор значений использовался и для YAML-подстановки, и для runtime узла.
...
- Проверенный commit:
  - `68f1814`
- Подтверждено:
  - оба узла стартуют и отвечают по `/status`;
